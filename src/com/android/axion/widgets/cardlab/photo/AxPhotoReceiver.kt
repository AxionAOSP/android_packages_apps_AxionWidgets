...
class AxPhotoReceiver : AxionWidgetProvider() {
    ...
    override fun requiredProviders() = listOf(PhotoProvider::class)

    companion object {
        fun update(context: Context, data: PhotoWidgetData) {
            val views = PhotoInteractor(context).updateWidget(data.widgetId, data.bitmap)
            AppWidgetManager.getInstance(context).updateAppWidget(data.widgetId, views)
        }
    }
}
